...
function Home() {
  ...
  let manageUsersMenu;
  if (useValidateUserHasAnyRole(['ROLE_TENANT_ADMIN', 'ROLE_USER_ADMIN', 'ROLE_USER_READER'])) {
    manageUsersMenu = (
  ...
  if (useValidateUserHasAnyRole(['ROLE_TENANT_ADMIN', 'ROLE_USER_ADMIN', 'ROLE_USER_READER', 'ROLE_USER']) === false) {
    projectView = null;
  } else {
    if (projectId) {
      projectView = (
